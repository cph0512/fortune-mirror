...
- keyPoints：三個最關鍵的點。**每個點 MUST 直接描述對題目本身的影響**，不能只丟宮位名或抽象吉凶標籤。
  ✗ 錯誤範例（問吃飯 vs 吃麵）：「福德宮有好運加持」「遷移宮動能活躍」「交友宮貴人助力」（完全沒講吃飯/吃麵，且暴露宮位術語）
  ✓ 正確範例（問吃飯 vs 吃麵）：「今天的心情適合慢慢享受正餐，不要急著趕回去」「晚餐氛圍比速食更能修復今天的疲勞」「這家店你之前吃過留下好印象，穩當選擇」
  ✗ 錯誤範例（問接 offer）：「官祿宮有化權」「財帛宮化忌」
  ✓ 正確範例（問接 offer）：「這家公司的文化跟你習慣的節奏對不上，進去後容易消耗」「薪資條件對當前階段是合理升幅」「直屬主管在你的貴人磁場上」
- analysis：至少 4 段，每層一段。若 timed 再加第 4 段談那個具體日時。
  - **每段都必須把能量判斷翻譯成「對這個選項本身的實際影響」**。禁止停留在「某宮有某化」這種內部語言。
...
表達風格：
- 禁止出現任何命理系統名稱（紫微、八字、星盤、命盤、排盤）或專有術語（四化、化祿、化權、化科、化忌、宮位、相位、星座、主星）。
- **宮位名稱是禁用術語**，不可出現：命宮、兄弟宮、夫妻宮、子女宮、財帛宮、疾厄宮、遷移宮、交友宮（僕役宮）、官祿宮、田宅宮、福德宮、父母宮。若要描述該領域，用自然語言取代：
  - 命宮 → 自我狀態、核心能量
  - 兄弟宮 → 手足/同事/合作夥伴關係
  - 夫妻宮 → 親密關係
  - 子女宮 → 小孩或創作產出
  ...
  - 福德宮 → 精神狀態、享受與興趣
  - 父母宮 → 長輩、上司、權威關係
- 自然語言、日常口吻，像朋友在給建議。禁止「有好運加持」「動能活躍」「貴人助力」這種含糊的算命腔，都要具體到場景。
- 使用者語言為 "${langLabel}"，所有 label、keyPoints、analysis、notes、recommendation 都必須是此語言。

相關性規則（極重要，違反會被判為失敗輸出）：
- 每條 keyPoint、每段 analysis **必須跟題目主題直接相關**。例如問吃飯就談飲食/心情/聚餐/預算/時間；問 offer 就談公司文化/工作內容/收入/通勤/下一步發展；問搬家就談居住空間/通勤距離/社區/孩子。
- 禁止把抽象能量標籤（「貴人加持」「動能強」「有阻礙」）當成理由直接拋給使用者，必須翻成具體場景。
- 若題目太瑣碎（吃飯/剪頭髮這類），依然按規則輸出，但分析篇幅可以精簡、直接、不硬撐 4 層；重點是提供真正有用的生活建議。
...
export default function WizardApp({ auth, onBack, onLogout }) {
  ...
  // parseQuestionDateTime is imported from ./date-parser.js so it can be
  // unit-tested without mounting React. See src/date-parser.js for docs.
